...
class Image(object):
    ...
    def get_info(self):
        ...
        info = {
            ...
            "creation": 'unknown ctime',
            "filename": 'unknown filename',
            "size": 'unknown size',
        }
        try:
            info["content_type"] = self.blob.content_type
            info["creation"] = self.blob.creation
            info["filename"] = self.blob.filename
            info["size"] = self.blob.size
        except Exception, e:
            logging.error("Failed to get image from blobstore: %s", type(e))
