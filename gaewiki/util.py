...
_BROKEN_IMAGE_URL = "<a href='%s' title='%s'><img alt='Image Broken - %s'></a>"
...
def render_image(args, title):
    ...
    try:
      img_url = img.get_url(size, crop)
    except:
      return _BROKEN_IMAGE_URL % ('/', title, 'GET_URL failed')

    try:
      img_filename = img.get_filename()
    except:
      return _BROKEN_IMAGE_URL % (img_url, title, 'GET_FILENAME failed')

    img_filename = cgi.escape(img_filename)
    attrs = "src='%s' alt='%s'" % (img_url, img_filename)
    ...
    if not title:
      ...
      target = '/w/image/view?key=%s' % img.get_key()
    else:
      target = pageurl(title)

    return "<a href='%s' title='%s'><img %s/></a>" % (target, title, attrs)
